...
class Tracking(Thread):
    ...
    def find_marker(self, frame, RESCALE=4):
        ...
        blur = cv2.GaussianBlur(
            frame_small, (int(127 / RESCALE), int(127 / RESCALE)), 0
        )
        ...
        mask = (diff[:, :, 0] > 60) & (diff[:, :, 2] > 60) & (diff[:, :, 1] > 50)
    ...
    def tracking(self):
        ...
        while self.running:
            ...
            if self.flag_reset_flow:
                self.flag_reset_flow = False
                last_Cx = Cx.copy()
                last_Cy = Cy.copy()
            ...
            self.slip_index_realtime = float(
                np.mean(((Cx - last_Cx) ** 8 + (Cy - last_Cy) ** 8)) ** 0.125
            )

            # self.slip_index_realtime = float(
            #     (np.mean(Cx - last_Cx) ** 4 + np.mean(Cy - last_Cy) ** 4) ** 0.25
            # )
            # self.slip_index_realtime = (
            #     float(
            #         np.mean((Cx[:, :-2] - last_Cx[:, :-2])) ** 2
            #         + np.mean((Cy[:, :-2] - last_Cy[:, :-2])) ** 2
            #     )
            #     ** 0.5
            # )
            # print("self.slip_index_realtime", self.slip_index_realtime)
