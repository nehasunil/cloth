...
def select_grasp(
    pred, feeding_mask, surface_normal, num_neighbour=100, grasp_target="edges"
):
    ...
    xx, yy = np.meshgrid([x for x in range(im_width)], [y for y in range(im_height)])
    ...
    _, lbl = cv2.distanceTransformWithLabels(
        inner_edges_filt.astype(np.uint8),
        cv2.DIST_L2,
        5,
    ...
    edge_confidence = pred[:, :, 1]

    outer_pts = np.array([yy_o, xx_o]).T
    inner_pts = direction[yy_o, xx_o]
    vs = inner_pts - outer_pts
    magns = np.linalg.norm(vs, axis=1)
    magns[magns == 0] = 1.0
    unitvs = vs / magns[:, None]
    originv = [0, 1]
    angles = np.arccos(np.dot(unitvs, originv))
    # angles[vs[:,0]<0] *= -1
    angles_mask = angles
    # angles_mask[(angles_mask > np.pi/4) & (angles_mask < np.pi*3/4)]= 0.
    angles_mask[(angles_mask < np.pi * 3 / 4)] = 0.0
    angles_mask[angles_mask > 0] = 1.0

    angle_mask_vis = np.zeros(segmentation.shape[:2])
    angle_mask_vis[segmentation[:, :, 1] == 255] = angles_mask == 1
    # cv2.imshow("angle_mask", angle_mask_vis)
    # cv2.waitKey(1)

    # cost = k_var * var + k_dist_right * dist_right + k_normal_z * np.abs(surface_normal_o_z)
    cost = (
        k_var * var
        + k_dist_right * dist_right
        + k_normal_z * np.abs(surface_normal_o_z)
        + k_segmentation * edge_confidence[segmentation[:, :, 1] == 255]
    )  # + k_angles * angles_cost

    print("COST min max", cost.min(), cost.max())  # 0.9~3.4
    cost = np.clip(1.5 - cost / 2, 0, 1)

    feeding_mask_outer_edge = feeding_mask[segmentation[:, :, 1] == 255]

    cost_mask = feeding_mask_outer_edge * angles_mask
    # COST_MAX = 1e7
    cost[cost_mask == 0] = 0
    # cost = cost * mask_outer_edge * angles_mask

    cost_vis = np.ones(segmentation.shape[:2]) * 0
    cost_vis[segmentation[:, :, 1] == 255] = cost

    # if len(cost_vis[cost_vis != COST_MAX]) > 0:
    #     cmax = cost_vis[cost_vis != COST_MAX].max()
    # else:
    #     cmax = COST_MAX

    # cost_vis = (cost_vis - cost_vis.min()) / (cmax - cost_vis.min() + 1e-6)

    # cv2.imshow("cost", cost_vis)
    # cv2.waitKey(1)

    # Choose min var point
    cost_ind_tuple = [(val, i) for i, val in enumerate(cost)]
    cost_sort = sorted(cost_ind_tuple)
    k = min(random.randint(0, 9), len(cost) - 1)
    cost_ind = cost_sort[k][1]
    ...
    # predicting angle
    temp, lbl = cv2.distanceTransformWithLabels(
        inner_edges.astype(np.uint8), cv2.DIST_L2, 5, labelType=cv2.DIST_LABEL_PIXEL
    )
...
def get_wrinkle(im):
    ...
    ddepth = cv2.CV_64F
    kernel_size = 5
    laplacian = cv2.Laplacian(-im, ddepth, ksize=kernel_size)
    ...
    for angle in angles:
        im2 = rotate(laplacian, angle, mode="edge")
        grasp_quality = convolve2d(im2, kernel, boundary="symm", mode="same")
        grasp_quality_back = rotate(grasp_quality, -angle, mode="edge")
        grasp_quality_list.append(grasp_quality_back)
    #     imshow(im2)
    #     plt.show()

    grasp_quality_list = np.array(grasp_quality_list)
    d = 2
    rescale = 6
    ...
    for i in range(laplacian.shape[0]):
        for j in range(laplacian.shape[1]):
            if laplacian[i, j] < laplacian.max() * 0.5:
                continue
            t = (
                -np.pi / 2
                + angles[np.argmax(grasp_quality_list[:, i, j])] / 180.0 * np.pi
            )
            res.append([i, j, t, im[i, j]])
    #         t = angles[np.argmin(grasp_quality_list[:, i, j])] / 180. * np.pi

    # plt.imshow(im_display)
    return res


def draw_wrinkle_angle(im, i, j, t):
    # draw the orientation for the wrinkle
    d = 2
    rescale = 6

    im = (im - im.min()) / (im.max() - im.min())
    im_display = cv2.resize(im, (0, 0), fx=rescale, fy=rescale)
    im_display = np.dstack([im_display, im_display, im_display])

    start_point = (
        int(rescale * (0.5 + j - d * np.cos(t))),
        int(rescale * (0.5 + i - d * np.sin(t))),
    )
    end_point = (
        int(rescale * (0.5 + j + d * np.cos(t))),
        int(rescale * (0.5 + i + d * np.sin(t))),
    ...
    color = (0, 0, 255)
    thickness = 1
    cv2.line(im_display, start_point, end_point, color, thickness)
